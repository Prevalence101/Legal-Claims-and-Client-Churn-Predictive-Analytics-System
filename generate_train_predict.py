...
# --- DataFrames generated from data generation (needed for prediction functions too) ---
...
# --- Data Preprocessing for Models (needed for prediction functions too) ---
...
# Features for churn prediction (globally accessible)
features_churn = [
...
def predict_churn(client_data: dict, models_dir, features_churn, categorical_features_churn):
    model_pipeline = joblib.load(os.path.join(models_dir, 'churn_prediction_model.pkl'))
...
def predict_claims_submission(client_data: dict, models_dir, features_claims_forecast, categorical_features_claims_forecast):
    model_pipeline = joblib.load(os.path.join(models_dir, 'claims_forecast_model.pkl'))
    input_df = pd.DataFrame([client_data])
    ...
    if 'total_interactions' not in input_df.columns:
        input_df['total_interactions'] = 0.0 # Add as float for consistency with other numericals
...
def predict_segmentation(client_data: dict, models_dir, features_list):
    # Models are loaded from MODELS_DIR directly within this function
    model_kmeans = joblib.load(os.path.join(models_dir, 'client_segmentation_model.pkl'))
    scaler = joblib.load(os.path.join(models_dir, 'scaler_segmentation.pkl'))
...
# VADER is rule-based and doesn't need to be saved/loaded like sklearn models.
...
def predict_risk_score(client_data: dict, models_dir,
    ...
    churn_model_pipeline = joblib.load(os.path.join(models_dir, 'churn_prediction_model.pkl'))
    segmentation_model = joblib.load(os.path.join(models_dir, 'client_segmentation_model.pkl'))
    scaler_segmentation = joblib.load(os.path.join(models_dir, 'scaler_segmentation.pkl'))
    risk_model_pipeline = joblib.load(os.path.join(models_dir, 'risk_scoring_model.pkl'))
    ...
    all_expected_cols = list(set(features_churn + categorical_features_churn + features_segmentation + features_risk + categorical_features_risk + 
                                 ['registrationDate', 'lastClaimDate', 'last_interaction_date', 'totalClaims', 'averageClaimAmount', 'total_interactions', 'feedbackText']))
    for col in all_expected_cols:
        if col not in input_df_single.columns:
            if col in ['registrationDate', 'lastClaimDate', 'last_interaction_date']:
                input_df_single[col] = pd.NaT # Use Pandas NaT for datetime NaNs
            elif col == 'feedbackText': # Handle optional feedback text
                input_df_single[col] = ''
            elif col in ['totalClaims', 'total_interactions', 'age', 'engagementScore', 'churnProbability', 'riskScore', 'averageClaimAmount', 'predicted_churn_prob', 'predicted_segment_cluster', 'avg_sentiment_compound']:
    ...
    input_df_single['recency_last_claim'] = (datetime.now() - input_df_single['lastClaimDate']).dt.days.fillna(-1)
    input_df_single['recency_registration'] = (datetime.now() - input_df_single['registrationDate']).dt.days
    ...
    input_df_single['recency_last_interaction'] = (datetime.now() - input_df_single['last_interaction_date']).dt.days.fillna(-1)

    # Predict churn probability (internal to risk score calculation)
    ...
    # Predict segment cluster (internal to risk score calculation)
    segment_data = input_df_single[features_segmentation]
    segment_data_values = segment_data.values.reshape(1, -1) # Reshape for scaler
    segment_data_scaled = scaler_segmentation.transform(segment_data_values)
    predicted_segment_cluster = segmentation_model.predict(segment_data_scaled)[0]
    input_df_single['predicted_segment_cluster'] = predicted_segment_cluster

    # Sentiment analysis (internal to risk score calculation)
    if client_data.get('feedbackText'):
...
def predict_fraud(claim_data: dict, models_dir, preprocessor_path, features_list, categorical_features_list):
    model_fraud = joblib.load(os.path.join(models_dir, 'fraud_detection_model.pkl'))
    preprocessor = joblib.load(os.path.join(models_dir, 'preprocessor_fraud.pkl')) # Load preprocessor here
    ...
    # For consistent scoring, use a fixed min/max or pass them as arguments
...
def recommend_retention_actions(client_id: str, models_dir, N=3):
    ...
    model_cf = joblib.load(os.path.join(models_dir, 'collaborative_filtering_model.pkl'))
...
if __name__ == "__main__":
    ...
    # --- 1. Client Churn Prediction Model (XGBoost) Training ---
    ...
    churn_pred = predict_churn(example_client_data_churn, MODELS_DIR, features_churn, categorical_features_churn)
    print(f"\nExample Churn Prediction for a new client: {churn_pred}")


    # --- 2. Claims Submission Forecasting Model (XGBoost) Training ---
    ...
    claims_pred = predict_claims_submission(example_client_data_claims, MODELS_DIR, features_claims_forecast, categorical_features_claims_forecast)
    print(f"\nExample Claims Submission Prediction for a new client: {claims_pred}")


    # --- 3. Client Segmentation Model (K-Means) Training ---
    ...
    analyzer = SentimentIntensityAnalyzer() # Re-initialize for direct run if needed
    ...
    # The vader_label function is already defined globally.
    df_feedback['vader_labeled_sentiment'] = df_feedback['compound_score'].apply(
    ...
    # --- 5. Risk Scoring Model (Ensemble Model: XGBoost + Clustering + NLP) Training ---
    print("\n--- Training Risk Scoring Model ---")

    df_risk_data = df_clients_enriched.copy()

    # Load pipelines for internal use during training
    ...
    # Handle missing values before scaling for segmentation prediction
    temp_segment_data_for_pred = df_risk_data[features_segmentation].fillna(0)
    df_risk_data['predicted_segment_cluster'] = segmentation_model_loaded.predict(
        scaler_segmentation_loaded.transform(temp_segment_data_for_pred)
    )
    ...
    # --- 6. Fraud Detection Model (Isolation Forest) Training ---
    ...
    preprocessor_fraud.fit(X_fraud) # Fit preprocessor here
    ...
    # The preprocessor_fraud needs to be loaded/defined before these example calls
    _temp_preprocessor_fraud = joblib.load(f"{MODELS_DIR}/preprocessor_fraud.pkl") # Load for example calls

    fraud_pred_high = predict_fraud(example_claim_data_fraud, MODELS_DIR, _temp_preprocessor_fraud, features_fraud, categorical_features_fraud)
    fraud_pred_low = predict_fraud(example_claim_data_normal, MODELS_DIR, _temp_preprocessor_fraud, features_fraud, categorical_features_fraud)
    ...
    # --- 7. Real-Time Recommendation Model (Collaborative Filtering) Training ---
    ...
    df_interactions_cf['action_type_encoded'] = df_interactions_cf['actionType'].map(action_type_to_encoded).fillna(-1).astype(int)
